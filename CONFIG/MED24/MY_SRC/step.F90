MODULE step
   ...
   USE obsmisfits
   USE lib_mpp !![DRU] need to access mpi_comm_opa
...
CONTAINS
   ...
   SUBROUTINE stp( kstp )
      ...
      !! ---------------------------------------------------------------------
#ifdef key_misfit
      real(wp)  :: tms, tme, tim1, tim2
      integer   :: iis, iie, ijs, ije
      integer   :: iext, jext, migo, mjgo
! OceanVar
      real(wp), ALLOCATABLE  :: tdep(:,:,:)
      real(wp), ALLOCATABLE  :: udep(:,:,:)
      real(wp), ALLOCATABLE  :: vdep(:,:,:)
      real(wp), ALLOCATABLE  :: del_z(:)
#endif
#if defined key_agrif
...
#ifdef key_misfit
! - Calculate misfits at each time step
      if(kstp.eq.nit000)then
       tms = 0.0
       tme = (nitend-nit000+1)*rdt/(24.*3600.)

        ALLOCATE ( tdep(jpi,jpj,jpk) )
        ALLOCATE ( udep(jpi,jpj,jpk) )
        ALLOCATE ( vdep(jpi,jpj,jpk) )
        ALLOCATE ( del_z(jpk) )
      ...
      if(jpjdta.lt.mjg(jpj))then
       ...
       call ini_msf_oceanvar( tms, tme, jpi, jpj, jpk, gdept_0, del_z, 1.0,   &                      
                       tmask, umask, vmask, glamt, gphit, tdep, udep, vdep,   &        
                       2, jpi-iext, 2, jpj-jext, mig(1), migo, mjg(1), mjgo )
!                         2, jpi-1, 2, jpj-1, mig(1), mig(jpi), mjg(1), mjg(jpj))

        call upd_fld_oceanvar( jpi, jpj, jpk,  gdept_0, tmask, tdep, sshb, tsb(1,1,1,jp_tem), tsb(1,1,1,jp_sal),   &
                         2, jpi-1, 2, jpj-1, mig(2), mig(jpi-1), mjg(2), mjg(jpj-1) )
        call upd_fld_oceanvar( jpi, jpj, jpk,  gdept_0, tmask, tdep, sshn, tsn(1,1,1,jp_tem), tsn(1,1,1,jp_sal),   &
                         2, jpi-1, 2, jpj-1, mig(2), mig(jpi-1), mjg(2), mjg(jpj-1) )
       ...
       tim1 = ((kstp-nit000+1)-1)*rdt/(24.*3600.)
       tim2 = (kstp-nit000+1)*rdt/(24.*3600.)

 !    call get_msf_oceanvar( tim1, tim2, jpi, jpj, jpk, sshb, tsb(1,1,1,jp_tem), tsb(1,1,1,jp_sal), ub, vb)

     IF( ln_apr_dyn ) THEN

!      call get_msf_oceanvar( tim1, tim2, jpi, jpj, jpk, sshb+ssh_ib/1000./9.81, tsb(1,1,1,jp_tem), tsb(1,1,1,jp_sal), ub, vb)
      call get_msf_oceanvar( tim1, tim2, jpi, jpj, jpk, sshb, tsb(1,1,1,jp_tem), tsb(1,1,1,jp_sal), ub, vb)

     ELSE

      call get_msf_oceanvar( tim1, tim2, jpi, jpj, jpk, sshb-ssh_ib, tsb(1,1,1,jp_tem), tsb(1,1,1,jp_sal), ub, vb)
!      call get_msf_oceanvar( tim1, tim2, jpi, jpj, jpk, sshb, tsb(1,1,1,jp_tem), tsb(1,1,1,jp_sal), ub, vb)

     ENDIF

      if(kstp.eq.nitend) call wrt_msf_oceanvar (mpi_comm_opa)
...
#  endif
      ...
      IF( ln_obsmisfits    )   CALL obs_misfits( kstp )
