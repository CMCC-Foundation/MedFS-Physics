...
SUBROUTINE NemoWwHandshake(sdt_n,sdu_n,sdv_n,cds_n,sdt_w,sdu_w,sdv_w,cds_w,gdxd,gdyd,nsb,sdxd,sdyd,ldi,ldj,nsea,iaproc,naproc,mapsf)
   ...
   REAL(wp), DIMENSION(:,:), ALLOCATABLE :: ld_vel_in !!matrix local domain of interpolated velocity at surface [GIR]
   REAL(wp), DIMENSION(:,:), ALLOCATABLE :: ld_vel_out
...
!!--------------------------------------
   ...
   IF (  (.not. mpprank_other_master_global == -1) .and. (.not. isMaster) .and. ModelKind == NemoWw_WW ) THEN
      ...
      PRINT *, "NemoWwCoupling : WW not master ",mpprank_glob,mpprank_model,iaproc,mppsize_model,naproc,nseal_c
...
!!--------------------------------------
   ...
   IF (  (.not. mpprank_other_master_global == -1) .and. isMaster .and. ModelKind == NemoWw_NEMO ) THEN
      ...
      ALLOCATE(ld_vel_in(gdxd,gdyd))
      ALLOCATE(ld_vel_out(gdxd,gdyd))
      DO i=1,gdxd
         DO j=1,gdyd
            ld_vel_in(i,j)=gdsu(IndexMat2Vec(i,j,gdxd))
         END DO
      END DO
      CALL InterpU(ld_vel_in(:,:),ld_vel_out(:,:),gdxd,gdyd,1,1)
      DO i=1,gdxd
         DO j=1,gdyd
            gdsu(IndexMat2Vec(i,j,gdxd))=ld_vel_out(i,j)
         END DO
      END DO
      DEALLOCATE(ld_vel_in)
      DEALLOCATE(ld_vel_out)
      ...
      DO i=1,gdxd
         ...
         END DO
      END DO
      CALL InterpV(ld_vel_in(:,:),ld_vel_out(:,:),gdxd,gdyd,1,1)
      DO i=1,gdxd
         DO j=1,gdyd
            gdsu(IndexMat2Vec(i,j,gdxd))=ld_vel_out(i,j)
         END DO
...
!!------------------------------------
   ...
   IF (  (.not. mpprank_other_master_global == -1) .and. isMaster .and. ModelKind == NemoWw_WW ) THEN
      ...
      DO p=1,mppsize_model-1
         ...
         CALL MPI_ERRCHECK(ierr)
         PRINT *,"NemoWwCoupling : WW master ric",mpprank_glob,mpprank_model,iaproc,mppsize_model,naproc,p+1,nseal_c
      ...
      PRINT *, "NemoWwCoupling : WW send CDS start",mpprank_other_master_global,CommGlob,size(gdsu)
