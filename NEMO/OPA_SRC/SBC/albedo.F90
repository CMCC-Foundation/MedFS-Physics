MODULE albedo
   ...
   !! $Id: albedo.F90 6316 2016-02-15 13:35:37Z cetlod $
...
CONTAINS
   ...
   SUBROUTINE albedo_ice( pt_ice, ph_ice, ph_snw, pa_ice_cs, pa_ice_os )
      ...
      REAL(wp), PARAMETER ::   ralb_oce = 0.066   ! ocean or lead albedo (Pegau and Paulson, Ann. Glac. 2001)
         ...
         DO jl = 1, ijpl
            DO jj = 1, jpj
               DO ji = 1, jpi
                  ...
                  zswitch = MAX( 0._wp , SIGN( 1._wp , ph_snw(ji,jj,jl) - c1 ) )
                  zalb_sf = ( 1._wp - zswitch ) * ( zalb_it(ji,jj,jl) + ph_snw(ji,jj,jl) &
                     &                          * ( ralb_sf - zalb_it(ji,jj,jl) ) * z1_c1 )  &
                     &     +          zswitch   * ralb_sf  
                  ...
                  zalb_sm = ( 1._wp - zswitch ) * ( ralb_im + ph_snw(ji,jj,jl) &
                     &                          * ( ralb_sm - ralb_im ) * z1_c2 )   &
                     &     +          zswitch   *   ralb_sm 
                  
                  ! snow albedo
                  zswitch = MAX( 0._wp , SIGN( 1._wp , pt_ice(ji,jj,jl) - rt0_snow ) )   
                  zalb_st = zswitch * zalb_sm + ( 1.0 - zswitch ) * zalb_sf
                  
                  ! Ice/snow albedo   
                  zswitch             = MAX( 0._wp , SIGN( 1._wp , - ph_snw(ji,jj,jl) ) )
                  pa_ice_cs(ji,jj,jl) = ( 1._wp - zswitch ) * zalb_st + zswitch *  zalb_it(ji,jj,jl)
               END DO
            END DO
         END DO
...
! 
   ...
   SUBROUTINE albedo_oce( pa_oce_os , pa_oce_cs )
      ...
      !!----------------------------------------------------------------------
      !
      pa_oce_cs(:,:) = 0.05 / ( 1.1 * rmue**1.4 + 0.15 )   ! Parameterization of Briegled and Ramanathan, 1982              
