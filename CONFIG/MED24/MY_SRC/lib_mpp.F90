...
!$AGRIF_END_DO_NOT_TREAT
   ...
   LOGICAL, PUBLIC                                  ::   ln_wrmppinit       ! namelist control for writing of mppinit file
...
CONTAINS
   ...
   FUNCTION mynode( ldtxt, ldname, kumnam_ref , kumnam_cfg , kumond , kstop, localComm )
      ...
      !
      NAMELIST/nammpp/ cn_mpi_send, nn_buffer, jpni, jpnj, jpnij,ln_nnogather,ln_wrmppinit
      !!----------------------------------------------------------------------
      !
      ii = 1
...
902   IF( ios /= 0 ) CALL ctl_nam ( ios , 'nammpp in configuration namelist', lwp )
      ...
      WRITE(ldtxt(ii),*) '      writing of mppinit file                       ln_wrmppinit = ', ln_wrmppinit ; ii = ii + 1
...
#if defined key_wwiii
            WRITE(ldtxt(ii),*) 'TEST 3 WRITE DRU'  ;   ii = ii + 1
#endif
         ENDIF
#if defined key_wwiii
         !CALL NemoWwInit(NemoWw_NEMO,1,mpi_comm_opa)
         WRITE(ldtxt(ii),"(a80)") NemoWwLog ; ii = ii + 1
#endif
      ELSE
         CALL mpi_comm_dup( mpi_comm_world, mpi_comm_opa, code)
#if defined key_wwiii
         !CALL NemoWwInit(NemoWw_NEMO,2,mpi_comm_opa)
         WRITE(ldtxt(ii),"(a80)") NemoWwLog ; ii = ii + 1
#endif
         IF( code /= MPI_SUCCESS ) THEN
            DO ji = 1, SIZE(ldtxt)
