...
# if defined key_mfs
      ! Modification in the computation of land-sea mask according to
      ! Dobricic for energy conservation
      ! P.Oddo (Dobricic-mask)
      ! ---------------------------------------     

        DO jj = 2, jpjm1
          DO ji = 2, jpim1
          IF((tmask(ji,jj,jk)+tmask(ji+1,jj,jk)+tmask(ji,jj+1,jk)+tmask(ji+1,jj+1,jk)).gt.2.5) THEN
                  fmask(ji,jj,jk) = 1.0
           ENDIF
          END DO
        END DO
