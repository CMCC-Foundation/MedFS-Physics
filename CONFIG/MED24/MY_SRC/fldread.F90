      ...
      IF( MOD( kt-1, kn_fsbc ) == 0 ) THEN         ! update field at each kn_fsbc time-step !
         ...
         DO jf = 1, imf                            ! ---   loop over field   --- !
            ...
            IF( sd(jf)%ln_tint ) THEN              ! temporal interpolation
               IF(lwp .AND. MOD(kt - 1, (86400/INT(rdt))/24 ) .EQ. 0 ) THEN
                  IF(lwp) WRITE(numout,*) 'SBC Temporal interpolation ON'
...
!CDIR COLLAPSE
            ...
            ELSE   ! nothing to do...
               IF(lwp .AND. MOD(kt - 1, (86400/INT(rdt))/4 ) .EQ. 0 ) THEN
                  IF(lwp) WRITE(numout,*) 'SBC Temporal interpolation OFF'
                  clfmt = "('fld_read: var ', a, ' kt = ', i8,' (', f9.4,' days), Y/M/D = ', i4.4,'/', i2.2,'/', i2.2," //   &
...
#ifdef key_mfs
         sdjf%nrec_a(1) = 2 + INT( ztmp ) - COUNT((/llbefore/))   ! record number to be read
#else
...
#ifdef key_mfs
             sdjf%nrec_a(2) = iendrec - ifreq_sec            ! swap at the middle of the record
#else
