...
CONTAINS
   ...
   SUBROUTINE sbc_ssr( kt )
      ...
      IF( nn_sstr + nn_sssr /= 0 ) THEN
         ...
         !
         isec = nsec_year + nsec1jan000  ! Get current time in seconds since Jan 1st nit000 (to compare with nrec_a/nrec_b)
         !
         !                                         ! ========================= !
         IF( MOD( kt-1, nn_fsbc ) == 0 ) THEN      !    Add restoring term     !
            ...
            IF( nn_sstr == 1 ) THEN                                   !* Temperature restoring term
               dt = MIN(ABS(isec - sf_sst(1)%nrec_a(2)),ABS(isec - sf_sst(1)%nrec_b(2)))
               IF ( nn_ssr_wgts == 1 ) THEN                              ! Apply gaussian weighted relaxation (with respect to observation time)
                  weight_sst = EXP(-(dt/(2*nn_hssr*3600))*(dt/(nn_hssr*3600)))
               ELSE IF ( nn_ssr_wgts == 2 ) THEN                         ! Apply cubic weighted relaxation (with respect to observation time)
                  weight_sst = EXP(-(dt/(2*nn_hssr*3600))*(dt/(nn_hssr*3600))*(dt/(nn_hssr*3600)))
               ELSE
                  weight_sst = 1.0
               ENDIF
               !
               IF ( nn_ssr_wgts > 0 .OR. dt < nn_hssr*3600 ) THEN        ! If nn_sstr==0, only do the restoring around observation date (nn_hssr hours)
                  IF ( ln_esst ) THEN                                           ! Only apply relaxation for error of SST < rn_esst
                     DO jj = 1, jpj
                        DO ji = 1, jpi
                           IF (sf_esst(1)%fnow(ji,jj,1) < rn_esst) THEN
                              zqrp = rn_dqdt * weight_sst * ( sst_m(ji,jj) - sf_sst(1)%fnow(ji,jj,1) )
                           ELSE
                              zqrp = 0.0
                           ENDIF
...
!CDIR COLLAPSE
   ...
   SUBROUTINE sbc_ssr_init
      ...
      CHARACTER(len=100) ::  cn_dir                ! Root directory for location of ssr files
      TYPE(FLD_N) ::   sn_sst, sn_sss, sn_esst     ! informations about the fields to be read
      NAMELIST/namsbc_ssr/ cn_dir, nn_sstr, nn_sssr, rn_dqdt, rn_deds, sn_sst, sn_esst, sn_sss, &
          &                ln_sssr_bnd, rn_sssr_bnd, nn_hssr, nn_ssr_wgts, ln_esst, rn_esst
      INTEGER     ::  ios
      !!----------------------------------------------------------------------
      !
...
902   IF( ios /= 0 ) CALL ctl_nam ( ios , 'namsbc_ssr in configuration namelist', lwp )
      ...
      IF( ln_esst ) THEN      !* set sf_esst structure & allocate arrays
         ...
         CALL fld_fill( sf_esst, (/ sn_esst /), cn_dir, 'sbc_ssr', 'SST restoring term toward SST data', 'namsbc_ssr' )
