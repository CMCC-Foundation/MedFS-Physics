MODULE fldread_bdy
   ...
   PUBLIC   fld_map_bdy    ! routine called by tides_init
   PUBLIC   fld_read_bdy, fld_fill_bdy   ! called by sbc... modules
   PUBLIC   fld_clopn_bdy
...
CONTAINS
   ...
   SUBROUTINE fld_read_bdy( kt, kn_fsbc, sd, map, kit, kt_offset )
      !!---------------------------------------------------------------------
      !!                    ***  ROUTINE fld_read  ***
      !!                   
      ...
      IF( ll_firstcall ) THEN                      ! initialization
         DO jf = 1, imf 
            ...
            CALL fld_init_bdy( kn_fsbc, sd(jf), imap )  ! read each before field (put them in after as they will be swapped)
         END DO
         IF( lwp ) CALL wgt_print_bdy()                ! control print
      ENDIF
      !                                            ! ====================================== !
      IF( MOD( kt-1, kn_fsbc ) == 0 ) THEN         ! update field at each kn_fsbc time-step !
         ...
         DO jf = 1, imf                            ! ---   loop over field   --- !
            ...
            IF( isecsbc > sd(jf)%nrec_a(2) .OR. ll_firstcall ) THEN    ! read/update the after data?
               ...
               CALL fld_rec_bdy( kn_fsbc, sd(jf), kt_offset = it_offset, kit = kit )    ! update after record informations
               ...
               IF( .NOT. ll_firstcall .AND. sd(jf)%ln_tint .AND. sd(jf)%nrec_b(1) /= sd(jf)%nreclast &
                  ...
                  CALL fld_get_bdy( sd(jf), imap )                  ! read after data
               ...
               CALL fld_clopn_bdy( sd(jf) )   ! Do we need to open a new year/month/week/day file?
               ...
               IF( sd(jf)%ln_tint ) THEN
                  ...
                  IF( sd(jf)%nrec_a(1) > sd(jf)%nreclast ) THEN
                     ...
                     IF( .NOT. ( sd(jf)%ln_clim .AND. sd(jf)%cltype == 'yearly' ) ) THEN   ! close/open the current/new file
                        ...
                        CALL fld_clopn_bdy( sd(jf), nyear  + COUNT((/llnxtyr /))                                           ,         &
                        ...
                        IF( sd(jf)%num <= 0 .AND. .NOT. llstop ) THEN    ! next year file does not exist
                           ...
                           CALL fld_clopn_bdy( sd(jf) )       ! back to the current year/month/day
               ...
               ! read after data
               CALL fld_get_bdy( sd(jf), imap )

            ENDIF   ! read new data?
         END DO                                    ! --- end loop over field --- !

         CALL fld_rot_bdy( kt, sd )                    ! rotate vector before/now/after fields if needed

         DO jf = 1, imf                            ! ---   loop over field   --- !
            !
            IF( sd(jf)%ln_tint ) THEN              ! temporal interpolation
               IF(lwp .AND. MOD(kt - 1, (86400/INT(rdt))/4 ) .EQ. 0 ) THEN
IF(lwp) WRITE(numout,*) 'BDY Temporal interpolation ON'
...
!IF(lwp) WRITE(numout,*) '~~~~~~~~~~~~~~~~~~~~'
!IF(lwp) WRITE(numout,*) 'isecsbc BDY is = ',isecsbc
!IF(lwp) WRITE(numout,*) '~~~~~~~~~~~~~~~~~~~~'
!IF(lwp) WRITE(numout,*) 'sd(jf)%nrec_b(2) BDY = ',sd(jf)%nrec_b(2)
!IF(lwp) WRITE(numout,*) '~~~~~~~~~~~~~~~~~~~~'
!IF(lwp) WRITE(numout,*) 'sd(jf)%nrec_a(2) BDY = ',sd(jf)%nrec_a(2)
!IF(lwp) WRITE(numout,*) '~~~~~~~~~~~~~~~~~~~~'
...
!CDIR COLLAPSE
            ...
            ELSE   ! nothing to do...
               IF(lwp .AND. MOD(kt - 1, (86400/INT(rdt))/4 ) .EQ. 0 ) THEN
IF(lwp) WRITE(numout,*) 'BDY Temporal interpolation OFF'
                  clfmt = "('fld_read: var ', a, ' kt = ', i8,' (', f9.4,' days), Y/M/D = ', i4.4,'/', i2.2,'/', i2.2," //   &
      ...
      !                                            ! ====================================== !
      !
   END SUBROUTINE fld_read_bdy


   SUBROUTINE fld_init_bdy( kn_fsbc, sdjf, map )
      !!---------------------------------------------------------------------
      !!                    ***  ROUTINE fld_init  ***
      !!
      ...
      CALL fld_rec_bdy( kn_fsbc, sdjf, ldbefore = .TRUE. )  ! return before values in sdjf%nrec_a (as we will swap it later)
      ...
      IF( sdjf%ln_tint ) THEN ! we need to read the previous record and we will put it in the current record structure
         ...
         !
         CALL fld_clopn_bdy( sdjf, iyear, imonth, iday, .NOT. llprev )
      ...
      ENDIF
      !
   END SUBROUTINE fld_init_bdy


   SUBROUTINE fld_rec_bdy( kn_fsbc, sdjf, ldbefore, kit, kt_offset )
      !!---------------------------------------------------------------------
      !!                    ***  ROUTINE fld_rec  ***
      !!
      !! ** Purpose : Compute
      ...
      ELSE                                   ! higher frequency mean (in hours)
         ...
         IF( sdjf%ln_tint ) THEN                ! time interpolation, shift by 1/2 record
            ...
            !                   
!IF(lwp) WRITE(numout,*) 'ln_tint TRUE BDY before ratio ztmp = ',ztmp
!IF(lwp) WRITE(numout,*) 'ln_tint TRUE BDY before ratio ifreq_sec = ',ifreq_sec
            ztmp= ztmp / REAL(ifreq_sec, wp) + 0.5
!IF(lwp) WRITE(numout,*) 'ln_tint TRUE BDY ztmp = ',ztmp
         ELSE                                   ! no time interpolation
            !
            !           INT( ztmp/ifreq_sec )
            !                     /|\
            ...
            !       forcing record :  1     2     3
            !                            
!IF(lwp) WRITE(numout,*) 'ln_tint FALSE BDY before ratio ztmp = ',ztmp
!IF(lwp) WRITE(numout,*) 'ln_tint FALSE BDY before ratio ifreq_sec = ',ifreq_sec
            ztmp= ztmp / REAL(ifreq_sec, wp)
!IF(lwp) WRITE(numout,*) 'ln_tint FALSE BDY ztmp = ',ztmp
         ENDIF

         sdjf%nrec_a(1) = 1 + INT( ztmp ) - COUNT((/llbefore/))   ! record number to be read
!IF(lwp) WRITE(numout,*) '~~~~~~~~~~~~~~~~~'
!IF(lwp) WRITE(numout,*) 'ln_tint TRUE BDY sdjf%nrec_a(1) = ',sdjf%nrec_a(1)
!IF(lwp) WRITE(numout,*) 'ln_tint TRUE BDY llbefore = ',COUNT((/llbefore/))
!IF(lwp) WRITE(numout,*) '~~~~~~~~~~~~~~~~~'

         iendrec = ifreq_sec * sdjf%nrec_a(1) + nsec1jan000       ! end of this record (in second)
!IF(lwp) WRITE(numout,*) '~~~~~~~~~~~~~~~~~'
!IF(lwp) WRITE(numout,*) 'iendrec BDY = ',iendrec
!IF(lwp) WRITE(numout,*) '~~~~~~~~~~~~~~~~~'
...
!IF(lwp) WRITE(numout,*) 'iendrec ln_tint TRUE BDY = ',iendrec
!IF(lwp) WRITE(numout,*) 'ifreq_sec ln_tint TRUE BDY = ',ifreq_sec
!IF(lwp) WRITE(numout,*) 'ln_tint TRUE BDY sdjf%nrec_a(2)  = ',sdjf%nrec_a(2)
...
!IF(lwp) WRITE(numout,*) 'iendrec ln_tint FALSE BDY = ',iendrec
!IF(lwp) WRITE(numout,*) 'ifreq_sec ln_tint FALSE BDY = ',ifreq_sec
!IF(lwp) WRITE(numout,*) 'ln_tint FALSE BDY sdjf%nrec_a(2) = ',sdjf%nrec_a(2)
!IF(lwp) WRITE(numout,*) 'ln_tint FALSE BDY sdjf%nrec_b(2) = ',sdjf%nrec_b(2)
         ENDIF
         !
      ENDIF
      !
   END SUBROUTINE fld_rec_bdy


   SUBROUTINE fld_get_bdy( sdjf, map )
      !!---------------------------------------------------------------------
      !!                    ***  ROUTINE fld_get  ***
      !!
      ...
      IF( ASSOCIATED(map%ptr) ) THEN
         IF( sdjf%ln_tint ) THEN   ;   CALL fld_map_bdy( sdjf%num, sdjf%clvar, sdjf%fdta(:,:,:,2), sdjf%nrec_a(1), map )
         ELSE                      ;   CALL fld_map_bdy( sdjf%num, sdjf%clvar, sdjf%fnow(:,:,:  ), sdjf%nrec_a(1), map )
         ENDIF
      ELSE IF( LEN(TRIM(sdjf%wgtname)) > 0 ) THEN
         CALL wgt_list_bdy( sdjf, iw )
         IF( sdjf%ln_tint ) THEN   ;   CALL fld_interp_bdy( sdjf%num, sdjf%clvar, iw , ipk  , sdjf%fdta(:,:,:,2), & 
              & sdjf%nrec_a(1), sdjf%lsmname )
         ELSE                      ;   CALL fld_interp_bdy( sdjf%num, sdjf%clvar, iw , ipk  , sdjf%fnow(:,:,:  ), &
              & sdjf%nrec_a(1), sdjf%lsmname )
         ENDIF
      ELSE
   ...
   END SUBROUTINE fld_get_bdy

   SUBROUTINE fld_map_bdy( num, clvar, dta, nrec, map )
      !!---------------------------------------------------------------------
      !!                    ***  ROUTINE fld_map  ***
      !!
   ...
   END SUBROUTINE fld_map_bdy


   SUBROUTINE fld_rot_bdy( kt, sd )
      !!---------------------------------------------------------------------
      !!                    ***  ROUTINE fld_rot  ***
      !!
      ...
      !
   END SUBROUTINE fld_rot_bdy


   SUBROUTINE fld_clopn_bdy( sdjf, kyear, kmonth, kday, ldstop )
      !!---------------------------------------------------------------------
      !!                    ***  ROUTINE fld_clopn  ***
      !!
      ...
      ENDIF
      !
   END SUBROUTINE fld_clopn_bdy


   SUBROUTINE fld_fill_bdy( sdf, sdf_n, cdir, cdcaller, cdtitle, cdnam )
      !!---------------------------------------------------------------------
      !!                    ***  ROUTINE fld_fill  ***
      !!
   ...
   END SUBROUTINE fld_fill_bdy


   SUBROUTINE wgt_list_bdy( sd, kwgt )
      !!---------------------------------------------------------------------
      !!                    ***  ROUTINE wgt_list  ***
      !!
      ...
      IF( .NOT.found ) THEN
         kwgt = nxt_wgt
         CALL fld_weight_bdy( sd )
      ENDIF
      !
   END SUBROUTINE wgt_list_bdy


   SUBROUTINE wgt_print_bdy( )
      !!---------------------------------------------------------------------
      !!                    ***  ROUTINE wgt_print  ***
      !!
      ...
      END DO
      !
   END SUBROUTINE wgt_print_bdy


   SUBROUTINE fld_weight_bdy( sd )
      !!---------------------------------------------------------------------
      !!                    ***  ROUTINE fld_weight  ***
      !!
      ...
      !
   END SUBROUTINE fld_weight_bdy


   SUBROUTINE apply_seaoverland_bdy(clmaskfile,zfieldo,jpi1_lsm,jpi2_lsm,jpj1_lsm, &
      ...
      CALL seaoverland_bdy(zfieldn,itmpi,itmpj,zfield)
      DO jc=1,nn_lsm
         CALL seaoverland_bdy(zfield,itmpi,itmpj,zfield)
      END DO
   ...
   END SUBROUTINE apply_seaoverland_bdy 


   SUBROUTINE seaoverland_bdy(zfieldn,ileni,ilenj,zfield)
      !!---------------------------------------------------------------------
      !!                    ***  ROUTINE seaoverland  ***
      !!
   ...
   END SUBROUTINE seaoverland_bdy


   SUBROUTINE fld_interp_bdy( num, clvar, kw, kk, dta,  &
                          &         nrec, lsmfile)      
      !!---------------------------------------------------------------------
      !!                    ***  ROUTINE fld_interp  ***
      !!
      ...
      !  Avoid out of bound
         ...
         CASE DEFAULT
              CALL iom_get( num, jpdom_unknown, clvar, ztmp_fly_dta(jpi1_lsm:jpi2_lsm,jpj1_lsm:jpj2_lsm,:),   &
                 &                                                                nrec, rec1_lsm, recn_lsm)
         END SELECT
         CALL apply_seaoverland_bdy(lsmfile,ztmp_fly_dta(jpi1_lsm:jpi2_lsm,jpj1_lsm:jpj2_lsm,:),                  &
