...
!
!ASA!/T      WRITE (NDST,9000) BETAMAX, USTAR, USDIR*RADE
!/T      WRITE (NDST,9000) USTAR, USDIR*RADE
!
! 1.  Preparations
!
...
!ASA MODIFICATION MFS-WAM (original Modificiation KORRES sub. stresso)
!      UST=USTP
      UST=MAX(USTP,0.000001)
...
!ASA MODIFICATION MFS-WAM (original Modificiation KORRES sub. stresso)
      XJ=(GRAV*Z0/UST**2-AALPHA) / DELALP
...
!
      ...
      DO I=0,ITAUMAX
         ...
         DO J=0,JUMAX
	       ...
	       ZZ0 = ZZ00/(1.-X)**XM
!ASA Modification for consistency with MFS-WAM 
...
!WRITE(101,'(20G16.8)') TAUHFT
      RETURN
!/T 9000 FORMAT (' TEST TABU_HF, L, K :',(2I4,3F8.3)/)    
        ...
        DO K=0,IUSTAR
          ...
          DO I=0,ILEVTAIL
            ...
            DO J=1,JTOT
             ...
             END DO
!ASA!/T      WRITE (NDST,9000) L,K,AALPHA+FLOAT(L)*DELALP,UST,TAUHFT(K,L,I)
!/T      WRITE (NDST,9000) L,K,AALPHA+FLOAT(L)*DELALP,UST,TAUHFT(K,L)
           END DO
        END DO
      DEALLOCATE(W)
...
!-----------------------------------------------------------------------------!
   ...
   !
      ...
      DELI1   = MIN(1.,XI - FLOAT(IND))  !interpolation coefficient for stress table
      DELI2   = 1. - DELI1
      XJ      = WINDSPEED/DELU
      J       = MIN ( JUMAX-1, INT(XJ) )
!ASA MODIFICATION MFS-WAM (original Modificiation KORRES sub. airsea)
! ASA     DELJ1   = XJ - FLOAT(J)
      DELJ1   = MIN(1.,XJ - FLOAT(J)) 
      DELJ2   = 1. - DELJ1
