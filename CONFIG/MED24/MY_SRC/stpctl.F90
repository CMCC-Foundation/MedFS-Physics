...
CONTAINS
   ...
   SUBROUTINE stp_ctl( kt, kindic )
      ...
      REAL(wp) ::   tocemax, tocemin, socemax, socemin ! temporary scalars
      REAL(wp) ::   tocemax_mpp, tocemin_mpp
      REAL(wp) ::   sshocemax     ! temporary scalars
      ...
      DO jk = 1, jpk
         DO jj = 1, jpj
            DO ji = 1, jpi
               ...
               tocemax = MAX(tocemax,tsn(ji,jj,jk,jp_tem))
               socemax = MAX(socemax,tsn(ji,jj,jk,jp_sal))
          END DO
        END DO
      END DO
      ...
      DO jk = 1, jpk
        DO jj = 2, jpjm1
          DO ji = 1, jpi
            IF( tmask(ji,jj,jk) == 1) tocemin = MIN(tocemin,tsn(ji,jj,jk,jp_tem))
            IF( tmask(ji,jj,jk) == 1) socemin = MIN(socemin,tsn(ji,jj,jk,jp_sal))
          END DO
        END DO
       END DO
      sshocemax = 0.e0
      DO jj = 2, jpjm1
        DO ji = 1, jpi
            IF( tmask(ji,jj,1) == 1) sshocemax = MAX(sshocemax,ABS(sshn(ji,jj)))
        END DO
      END DO

      IF( lk_mpp )   CALL mpp_max( zumax )                 ! max over the global domain
      IF( lk_mpp )   CALL mpp_max( tocemax )               ! max over the global domain
      IF( lk_mpp )   CALL mpp_max( socemax )               ! max over the global domain
      IF( lk_mpp )   CALL mpp_max( sshocemax )             ! max over the global domain
      !
      IF( MOD( kt, nwrite ) == 1 .AND. lwp )   WRITE(numout,*) ' ==>> time-step= ',kt,' abs(U) max: ', zumax
      IF( MOD( kt, nwrite ) == 1 .AND. lwp )   WRITE(numout,*) ' ==>> time-step= ',kt,' TOCE   max: ', tocemax
      IF( MOD( kt, nwrite ) == 1 .AND. lwp )   WRITE(numout,*) ' ==>> time-step= ',kt,' SOCE   max: ', socemax
      IF( MOD( kt, nwrite ) == 1 .AND. lwp )   WRITE(numout,*) ' ==>> time-step= ',kt,' SSHOCE max: ', sshocemax

      !
      IF( lk_mpp )   CALL mpp_min( tocemin )               ! min over the global domain
      IF( lk_mpp )   CALL mpp_min( socemin )               ! min over the global domain
      !
      IF( MOD( kt, nwrite ) == 1 .AND. lwp )   WRITE(numout,*) ' ==>> time-step= ',kt,' TOCE   min: ', tocemin
      IF( MOD( kt, nwrite ) == 1 .AND. lwp )   WRITE(numout,*) ' ==>> time-step= ',kt,' SOCE   min: ', socemin
      !
      IF( lwp )   WRITE(numout,*) ' ==>> time-step= ',kt,' abs(U) max: ', zumax
      IF( lwp )   WRITE(numout,*) ' ==>> time-step= ',kt,' TOCE   max: ', tocemax
      IF( lwp )   WRITE(numout,*) ' ==>> time-step= ',kt,' SOCE   max: ', socemax
      IF( lwp )   WRITE(numout,*) ' ==>> time-step= ',kt,' SSHOCE max: ', sshocemax
      IF( lwp )   WRITE(numout,*) ' ==>> time-step= ',kt,' TOCE   min: ', tocemin
      IF( lwp )   WRITE(numout,*) ' ==>> time-step= ',kt,' SOCE   min: ', socemin

      CALL FLUSH(numout)

      IF( tocemax > 40.e0 ) THEN
         IF( lk_mpp ) THEN
            CALL mpp_maxloc(tsn(:,:,:,jp_tem),tmask,tocemax_mpp,ii,ij,ik)
         ELSE
            ilocu = MAXLOC( tsn(:,:,:,jp_tem) )
            ii = ilocu(1) + nimpp - 1
            ij = ilocu(2) + njmpp - 1
            ik = ilocu(3)
         ENDIF
         IF(lwp) THEN
            ...
            WRITE(numout,*) ' stpctl: the temperature is larger than 40 degC'
            WRITE(numout,*) ' ====== '
            WRITE(numout,9302) kt, tocemax_mpp, ii, ij, ik
            WRITE(numout,*)
            WRITE(numout,*) '          output of last fields in numwso'
         ENDIF
         kindic = -3
      ENDIF
9302  FORMAT (' kt=',i6,' max T: ',1pg11.4,', i j k: ',3i5)

      IF( tocemin < 0.e0 ) THEN
         IF( lk_mpp ) THEN
            CALL mpp_minloc(tsn(:,:,:,jp_tem),tmask,tocemin_mpp,ii,ij,ik)
         ELSE
            ilocu = MAXLOC( tsn(:,:,:,jp_tem) )
            ii = ilocu(1) + nimpp - 1
            ij = ilocu(2) + njmpp - 1
         ...
         IF(lwp) THEN
            WRITE(numout,cform_err)
            WRITE(numout,*) ' stpctl: the temperature is lower than 0 degC'
            WRITE(numout,*) ' ====== '
            WRITE(numout,9301) kt, tocemin_mpp, ii, ij, ik
            WRITE(numout,*)
            WRITE(numout,*) '          output of last fields in numwso'
         ENDIF
         kindic = -3
      ENDIF
9301  FORMAT (' kt=',i6,' min T: ',1pg11.4,', i j k: ',3i5)
...
9500  FORMAT (' kt=',i6,' min SSS: ',1pg11.4,', i j: ',2i5)
      ...
      ELSE                                   !* ssh statistics (and others...)
         ...
         IF(.NOT. ABS(zssh2) < 1.e+20 .OR. &
            .NOT. ABS(zumax) < 1.e+20 .OR. &
            .NOT. ABS(zsmin) < 1.e+20 ) THEN
                kindic = -3
                IF(lwp) WRITE(numout,*) ' stpctl: problems with S,SSH,U val'
