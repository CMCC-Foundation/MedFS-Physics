...
!!...            &                ' not agree with Direct Initialization time' )
         ...
         IF ( ln_trainc ) THEN   
            CALL iom_get( inum, jpdom_autoglo, 'INCTEMPER', t_bkginc, 1 )
            CALL iom_get( inum, jpdom_autoglo, 'INCSALINE', s_bkginc, 1 )
            ! Apply the masks
         ...
         IF ( ln_sshinc ) THEN
            CALL iom_get( inum, jpdom_autoglo, 'INCSSHEIG', ssh_bkginc, 1 )
            ! Apply the masks
         ...
         IF ( kt == nitdin_r ) THEN
            ...
            IF (ln_temnofreeze) THEN
               ...
               WHERE( t_bkginc(:,:,:) > 0.0_wp .OR. tsn(:,:,:,jp_tem) + t_bkginc(:,:,:) > fzptnz(:,:,:) ) 
                  tsn(:,:,:,jp_tem) = tsn(:,:,:,jp_tem) + t_bkginc(:,:,:)   
               END WHERE
            ELSE
               tsn(:,:,:,jp_tem) = tsn(:,:,:,jp_tem) + t_bkginc(:,:,:)   
            ENDIF
            IF (ln_salfix) THEN
               ...
               WHERE( s_bkginc(:,:,:) > 0.0_wp .OR. tsn(:,:,:,jp_sal) + s_bkginc(:,:,:) > salfixmin ) 
                  tsn(:,:,:,jp_sal) = tsn(:,:,:,jp_sal) + s_bkginc(:,:,:)   
               END WHERE
            ELSE
               tsn(:,:,:,jp_sal) = tsn(:,:,:,jp_sal) + s_bkginc(:,:,:)   
            ENDIF
...
!!gm fabien            CALL eos( tsn, rhd )                      ! Compute rhd
   ...
   SUBROUTINE ssh_asm_inc( kt )
      ...
      ELSEIF ( ln_asmdin ) THEN
         ...
         IF(lwp) THEN
               WRITE(numout,*)
               WRITE(numout,*) 'ssh_asm_inc : Applying Analysis Increments',&
               & ' at time step = ', kt
               WRITE(numout,*) '~~~~~~~~~~~~'
            ENDIF
