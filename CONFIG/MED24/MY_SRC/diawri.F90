...
#  include "vectopt_loop_substitute.h90"
   ...
   !! $Id: diawri.F90 6315 2016-02-15 12:24:20Z cetlod $
   ...
   SUBROUTINE dia_wri( kt )
      ...
      IF( kt == nit000 ) THEN
         ...
         IF(  lk_vvl  ) THEN
            ...
            &             jpi, jpj, nh_T, ipk, 1, ipk, nz_T, 32, clop, zsto, zout )
            CALL histdef( nid_T, "vovvldept", "T point depth"                      , "m"     ,&  ! fsdept_n
            &             jpi, jpj, nh_T, ipk, 1, ipk, nz_T, 32, clop, zsto, zout )
         ...
         CALL histdef( nid_T, "sowaflup", "Net Upward Water Flux"              , "Kg/m2/s",   &  ! (emp-rnf)
            &          jpi, jpj, nh_T, 1  , 1, 1  , -99 , 32, clop, zsto, zout )
         CALL histdef( nid_T, "soevapor", "Water Evaporation Flux"             , "Kg/m2/s",   &  ! evap
            &          jpi, jpj, nh_T, 1  , 1, 1  , -99 , 32, clop, zsto, zout )
         CALL histdef( nid_T, "soprecip", "Precipitation Flux"                 , "Kg/m2/s",   &  ! prec
            &          jpi, jpj, nh_T, 1  , 1, 1  , -99 , 32, clop, zsto, zout )
         ...
         CALL histdef( nid_T, "soshfldo", "Shortwave Radiation"                , "W/m2"   ,   &  ! qsr
            &          jpi, jpj, nh_T, 1  , 1, 1  , -99 , 32, clop, zsto, zout )
         CALL histdef( nid_T, "solofldo", "Downward Longwave Radiation"        , "W/m2"   ,   &  ! -qbw
            &          jpi, jpj, nh_T, 1  , 1, 1  , -99 , 32, clop, zsto, zout )
         CALL histdef( nid_T, "sosefldo", "Downward Sensible Heat Flux"        , "W/m2"   ,   &  ! -ha
            &          jpi, jpj, nh_T, 1  , 1, 1  , -99 , 32, clop, zsto, zout )
         CALL histdef( nid_T, "solafldo", "Downward Latent   Heat Flux"        , "W/m2"   ,   &  ! -elat
            &          jpi, jpj, nh_T, 1  , 1, 1  , -99 , 32, clop, zsto, zout )
...
!
         ...
         IF(  lk_vvl  ) THEN
            CALL histdef( nid_W, "vovvldepw", "W point depth" , "m"     ,&                        ! fsdepw_n
            &             jpi, jpj, nh_W, ipk, 1, ipk, nz_W, 32, clop, zsto, zout )
         ENDIF
...
# endif
      ...
      IF( lk_vvl ) THEN
         ...
         CALL histwrite( nid_T, "vovvldept", it, fsdept_n(:,:,:) , ndim_T , ndex_T  )   ! t-point depth
         CALL histwrite( nid_W, "vovvldepw", it, fsdepw_n(:,:,:) , ndim_T , ndex_T  )   ! w-point depth
      ...
      CALL histwrite( nid_T, "soevapor", it, ( evap )      , ndim_hT, ndex_hT )   ! water evaporation flux
      CALL histwrite( nid_T, "soprecip", it, ( prec )      , ndim_hT, ndex_hT )   ! precipitation flux
      ...
      CALL histwrite( nid_T, "solofldo", it, -qbw          , ndim_hT, ndex_hT )   ! downward longwave radiation
      CALL histwrite( nid_T, "sosefldo", it, -ha           , ndim_hT, ndex_hT )   ! downward sensible heat flux
      CALL histwrite( nid_T, "solafldo", it, -elat         , ndim_hT, ndex_hT )   ! downward latent heat flux
...
!     IF( nn_timing == 1 )   CALL timing_start('dia_wri_state') ! not sure this works for routines not called in first timestep
      ...
      CALL histdef( id_i, "sometauy", "Meridional Wind Stress", "N/m2"   ,   &   ! j-wind stress
         &          jpi, jpj, nh_i, 1  , 1, 1  , -99 , 32, clop, zsto, zout )
      IF( lk_vvl ) THEN
         CALL histdef( id_i, "vovvldept", "T point depth"         , "m"      ,   &   ! t-point depth
            &          jpi, jpj, nh_i, jpk, 1, jpk, nz_i, 32, clop, zsto, zout )
         CALL histdef( id_i, "vovvldepw", "W point depth"         , "m"      ,   &   ! w-point depth
            &          jpi, jpj, nh_i, jpk, 1, jpk, nz_i, 32, clop, zsto, zout )
      ...
      CALL histwrite( id_i, "soevapor", kt, (evap )          , jpi*jpj    , idex )    ! water evaporation flux
      CALL histwrite( id_i, "soprecip", kt, (prec )          , jpi*jpj    , idex )    ! precipitation flux
      CALL histwrite( id_i, "sorunoff", kt, (rnf )           , jpi*jpj    , idex )    ! runoffs
      ...
      CALL histwrite( id_i, "solofldo", kt, -qbw             , jpi*jpj    , idex )    ! downward longwave radiation
      CALL histwrite( id_i, "sosefldo", kt, -ha              , jpi*jpj    , idex )    ! downward sensible heat flux
      CALL histwrite( id_i, "solafldo", kt, -elat            , jpi*jpj    , idex )    ! downward latent heat flux
      ...
      IF( lk_vvl ) THEN
         CALL histwrite( id_i, "vovvldept", kt, fsdept_n(:,:,:), jpi*jpj*jpk, idex )!  T-cell depth       
         CALL histwrite( id_i, "vovvldepw", kt, fsdepw_n(:,:,:), jpi*jpj*jpk, idex )!  T-cell depth  
