MODULE sbc_oce
   ...
   !!
   REAL(wp), PUBLIC, ALLOCATABLE, SAVE, DIMENSION(:,:) ::   qbw               !: downward longwave radiation                  [W/m2]
   REAL(wp), PUBLIC, ALLOCATABLE, SAVE, DIMENSION(:,:) ::   ha                !: downward sensible heat flux                  [W/m2]
   REAL(wp), PUBLIC, ALLOCATABLE, SAVE, DIMENSION(:,:) ::   elat              !: downward latent heat flux
   !!
   REAL(wp), PUBLIC, ALLOCATABLE, SAVE, DIMENSION(:,:) ::   evap              !: evaporation rate 
   REAL(wp), PUBLIC, ALLOCATABLE, SAVE, DIMENSION(:,:) ::   prec              !: precipitation rate 
   !!
   ...
   REAL(wp), PUBLIC, ALLOCATABLE, SAVE, DIMENSION(:,:) ::   cdn_wave          !: wave drag coefficient
   REAL(wp), PUBLIC, ALLOCATABLE, SAVE, DIMENSION(:,:) ::   dT_AS             !:AIR-SEA temperature difference
#endif
   !!----------------------------------------------------------------------
...
CONTAINS
   ...
   INTEGER FUNCTION sbc_oce_alloc()
      ...
      ALLOCATE( fwfisf  (jpi,jpj), rnf  (jpi,jpj) , sbc_tsc  (jpi,jpj,jpts) , qsr_hc  (jpi,jpj,jpk) ,     &
         ...
         !
      ALLOCATE( qbw (jpi,jpj)    , ha (jpi,jpj)     , elat (jpi,jpj) , STAT=ierr(4) )
      ALLOCATE( evap(jpi,jpj)    , prec (jpi,jpj)                    , STAT=ierr(5) )
      ALLOCATE( tprecip(jpi,jpj) , sprecip(jpi,jpj) , fr_i(jpi,jpj) ,     &
#if defined key_cpl_carbon_cycle
         &      atm_co2(jpi,jpj) ,                                        &
#endif
         &      ssu_m  (jpi,jpj) , sst_m(jpi,jpj) , frq_m(jpi,jpj) ,      &
         &      ssv_m  (jpi,jpj) , sss_m(jpi,jpj) , ssh_m(jpi,jpj) , STAT=ierr(6) )
         !
#if defined key_vvl
      ALLOCATE( e3t_m(jpi,jpj) , STAT=ierr(5) )
#endif
#if defined key_wwiii
      ALLOCATE( cdn_wave(jpi,jpj) , dT_AS(jpi,jpj) , STAT=ierr(7) ) 
      cdn_wave(:,:) = 0.0
      dT_AS(:,:) = 0.0
#endif
         !
