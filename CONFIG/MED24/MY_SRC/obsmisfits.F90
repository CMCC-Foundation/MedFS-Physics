...
#if defined NEMO_OLD
#define tsn(:,:,:,jp_tem) tn
#define tsn(:,:,:,jp_sal) sn
#define tsn(:,:,1,jp_tem) tn(:,:,1)
#define tsn(:,:,1,jp_sal) sn(:,:,1)
#endif
...
!... For IceVAR
   LOGICAL :: ln_icevar = .FALSE.
   INTEGER :: nn_iv_verb = 0, nn_istop_iv = 9999, nn_ivtime = 1, nn_iv_iau=40
   LOGICAL :: ll_iv_iau = .FALSE.
   LOGICAL :: ln_only_sic = .FALSE.
   INTEGER :: nn_thick_cons = 0
!... 
...
CONTAINS
   ...
   SUBROUTINE obs_misfits( kt )
      ...
      INTEGER :: nt , jobs , i1 , j1 , k1 , i2 , j2 , k2, kp, jk, knl,ji,jj
...
!          CALL prep_bc(kt, ln_noaa_sst, nn_sststart, nn_sstend)
      ...
      IF( ln_asmean ) CALL as_mean( kt, nobsm_freq, jpav, ln_meanvars )

      IF( .NOT. ln_meanvars(1) ) amts_m(:,:,:,1) = tsn(:,:,:,jp_tem)
      IF( .NOT. ln_meanvars(2) ) amts_m(:,:,:,2) = tsn(:,:,:,jp_sal)
      IF( .NOT. ln_meanvars(3) ) amh_m = sshn
      IF( .NOT. ln_meanvars(4) ) amuv_m(:,:,:,1) = un
      IF( .NOT. ln_meanvars(5) ) amuv_m(:,:,:,2) = vn
      IF( .NOT. ln_meanvars(6) ) amtq_m(:,:,1) = t_aml
      IF( .NOT. ln_meanvars(7) ) amtq_m(:,:,2) = q_aml
      ...
      IF( lk_mpp ) then
          CALL mpp_max(npobs)
          IF(npobs .gt. 0 .and. jpnij .gt. 1) CALL prep_mppmisfits
      ENDIF
      ...
      IF( ntobs(nt,NnINS) .GT. 0 ) THEN
        ...
        DO jobs=ntstart(nt,NnINS),ntend(nt,NnINS)
          ...
          IF( ins%kty(jobs) .LT. 5 ) THEN
             ...
             if(ln_omdebug) then
                WRITE(numuni,*) '*',jobs,ins%par(jobs),ins%sd1(jobs),ins%sd2(jobs),ins%bac(jobs),nimppt(narea),&
                & ins%ib(jobs,1:npq)-nimppt(narea)+1,ins%jb(jobs,1:npq)-njmppt(narea)+1,k1,kp,&
                & amts_m(ins%ib(jobs,1)-nimppt(narea)+1,ins%jb(jobs,1)-njmppt(narea)+1,k1,kp),&
                & amts_m(ins%ib(jobs,1)-nimppt(narea)+1,ins%jb(jobs,1)-njmppt(narea)+1,k2,kp)
             endif
             ! Compute pot. temp. from model salinity (neirest neighbour) if needed
      ...
      IF( ln_sshcorr .and. ll_slaobs ) THEN
          ...
          IF (lk_mpp) CALL mpp_sum( zsshcorr )
      ...
      IF( ntobs(nt,NnSLA) .GT. 0 ) THEN
        DO jobs=ntstart(nt,NnSLA),ntend(nt,NnSLA)
            ...
            sla%bac(jobs)  = osum(sla%pq(jobs,:),&
            & amh_m(sla%ib(jobs,:)-nimppt(narea)+1,sla%jb(jobs,:)-njmppt(narea)+1)) - &
                           osum(sla%pq(jobs,:),&
            & mdt(sla%ib(jobs,:)-nimppt(narea)+1,sla%jb(jobs,:)-njmppt(narea)+1))
          else
            sla%bac(jobs) = mpp_misfits_sla(sla%sd1(jobs),sla%sd2(jobs),sla%ib(jobs,:), sla%jb(jobs,:), &
            & sla%pq(jobs,:), sla%moi(jobs,:), sla%moj(jobs,:),sla%nind(jobs) )
          endif
...
!           if (sla%sd1(jobs) .eq. sla%sd2(jobs) ) then
!            DO jpred=1,nprd_sla
!             sla%bcp(jobs,jpred)  = osum(sla%pq(jobs,:),&
!             & preds(sla%ib(jobs,:)-nimppt(narea)+1,sla%jb(jobs,:)-njmppt(narea)+1,jpred))
!            ENDDO
!           else
            sla%bcp(jobs,:) = -9.E+12
!           endif
...
!           if( ln_prepbc ) then
!            DO jpred=1,nprd_sst
!             sst%bcp(jobs,jpred)  = osum(sst%pq(jobs,:),&
!             & preds(sst%ib(jobs,:)-nimppt(narea)+1,sst%jb(jobs,:)-njmppt(narea)+1,jpred))
!            ENDDO
!           endif
...
!            sss%bcp(jobs,4)  = osum(sss%pq(jobs,1:npq),&
!            & amts_m(sss%ib(jobs,1:npq)-nimppt(narea)+1,sss%jb(jobs,1:npq)-njmppt(narea)+1,1,1))
!            sss%bcp(jobs,5)  = osum(sss%pq(jobs,1:npq),&
!            & wndm(sss%ib(jobs,1:npq)-nimppt(narea)+1,sss%jb(jobs,1:npq)-njmppt(narea)+1))
!            sss%bcp(jobs,6)  = osum(sss%pq(jobs,1:npq),&
!            & x_dsst(sss%ib(jobs,1:npq)-nimppt(narea)+1,sss%jb(jobs,1:npq)-njmppt(narea)+1))
!            sss%bcp(jobs,7)  = osum(sss%pq(jobs,1:npq),&
!            & x_csdsst(sss%ib(jobs,1:npq)-nimppt(narea)+1,sss%jb(jobs,1:npq)-njmppt(narea)+1))
          else
      ...
      IF( ln_noaa_sst) THEN
        ...
        IF( nn_tmp .ge. nn_sststart .and. nn_tmp .le. nn_sstend ) THEN
          IF( .NOT. ln_smooth_noaa_sst ) THEN
            dsst = dsst + tsn(:,:,1,jp_tem)/REAL( nn_sstend-nn_sststart+1 , wp )
          ELSE
            nn_sstts = nn_sstts+1
            dsst = dsst + tsn(:,:,1,jp_tem)*sst_wgt(nn_sstts)
          ENDIF
        ENDIF
      ENDIF
      ...
      IF( ln_seaice ) THEN
        nn_tmp = kt - nit000 + 1
        rdiv = REAL( nn_sstend-nn_sststart+1 , wp )
        IF( nn_tmp .ge. nn_sststart .and. nn_tmp .le. nn_sstend ) THEN
            seaice(:,:,1) = seaice(:,:,1) + (1._wp-frld)/rdiv
            seaice(:,:,2) = seaice(:,:,2) + hicif/rdiv
            seaice(:,:,3) = seaice(:,:,3) + sist/rdiv
            seaice(:,:,4) = seaice(:,:,4) + hsnif/rdiv
            seaice(:,:,5) = seaice(:,:,5) + tsn(:,:,1,jp_tem)/rdiv
            seaice(:,:,6) = seaice(:,:,6) + tsn(:,:,1,jp_sal)/rdiv
        ENDIF
      ENDIF
#endif
   ...
   SUBROUTINE obsmis_init
      ...
      & ln_seaice, ln_icevar, nn_iv_verb, nn_istop_iv, nn_ivtime,ln_only_sic, &
      & ln_restdiff, nrd_ts, nn_rd_rest, nn_rd_restd,ll_iv_iau,nn_iv_iau, &
      & nn_thick_cons,ln_omdebug
      ...
      REWIND( numnam_ref )
      READ  ( numnam_ref , namobs_misfits )

      IF( .not. ln_obsmisfits ) RETURN

      ALLOCATE( dsst(jpi,jpj), e1e2_i(jpi,jpj) )

      ALLOCATE( amts_m(jpi,jpj,jpk,2) )
      ALLOCATE( amuv_m(jpi,jpj,jpk,2) )
      ALLOCATE( amh_m (jpi,jpj      ) )
      ALLOCATE( amtq_m(jpi,jpj,    2) )
      ...
      ln_sf = ( nbondj .le. -1 .OR. noso .lt. 0 )
      ln_nf = ( nbondj  .ge. 1 .OR. nono .lt. 0 )
      ln_wf = ( nbondi .le. -1 .OR. nowe .lt. 0 )
      ln_ef = ( nbondi  .ge. 1 .OR. noea .lt. 0 )
      ...
      IF( ( ln_noaa_sst .OR. ln_seaice ).AND. (nn_sststart.EQ.-1 .OR. nn_sstend.EQ.-1) ) THEN
      ...
      IF( ln_seaice ) THEN
         ALLOCATE( seaice(jpi,jpj,np_sip) )
         seaice (:,:,:) = 0._wp
      ENDIF
      ...
      if (ln_sshcorr ) THEN
        ...
        atot = SUM( e1e2_i(:,:) )
        IF( lk_mpp )   CALL  mpp_sum( atot )  ! sum over the global domain
...
!                 ENDIF
   ...
   SUBROUTINE obsmis_term
      ...
      IF( ln_seaice ) THEN
          WRITE(cfd,'(A,I4.4,A)') 'seaice_output_',narea-1,'.bin'
          OPEN(ufd,FILE=cfd,ACCESS='SEQUENTIAL',FORM='UNFORMATTED',&
          & STATUS='NEW')
          WRITE(ufd) jpi, jpj, 1, np_sip
          WRITE(ufd) REAL(seaice,KIND=sp)
          CLOSE(ufd) 
          DEALLOCATE( seaice )
      ENDIF
   ...
   SUBROUTINE readmdt(no_sla)
      ...
      IF( no_sla > 0 ) THEN
        ...
        CALL iom_get(inum,jpdom_autoglo,'mdt',mdt,1)
        CALL iom_close( inum )
      ENDIF
   ...
   SUBROUTINE prepobs
      ...
      ntst=INT((nitend-nit000+1)/nobsm_freq)
...
! Debugging version
   ...
   REAL(wp) FUNCTION mpp_misfits_sla(sd1,sd2,ib,jb,pq,mi,mj,ind)

   USE MPI

   IMPLICIT NONE

   INTEGER, INTENT(IN) :: sd1, sd2, ib(npq),jb(npq),mi(npq),mj(npq), ind
   REAL(wp), INTENT(IN):: pq(:)
    INTEGER  ::  ji, jj
   REAL(wp) :: zt, zff(2)
   INTEGER  :: jp, jp2, stat, ierr, nundb, i0,j0, iopt, ncc, nrr
   ...
   SUBROUTINE prep_mppmisfits
      ...
      if( .not. ln_wf ) THEN
        ...
        do jj=1,jpj
          do ji=1,3
            k=k+1
            west (k) = sshn(nc+ji-1,jj)-mdt(nc+ji-1,jj)
          enddo
        enddo
        do jp=1,2
            do jj=1,jpj
      ...
      ENDIF
      CALL mpi_isend( west, imwe, mpi_double_precision, nowe, tag1, mpi_comm_opa, isend_w, ierr)
      IF( llmpdebug ) THEN
      ...
      ENDIF
      CALL mpi_irecv(east,  imwe, mpi_double_precision, noea, mpi_any_tag, mpi_comm_opa, irecv_e, ierr) 
      IF( llmpdebug ) THEN
       WRITE(numuni,*) ' Done'  ; CALL FLUSH(numuni)
      ENDIF
      endif
      ...
      If( .not. ln_sf ) Then
        ...
        do jj=1,3
          do ji=1,jpi
            k=k+1
            south (k) = sshn(ji,nr+jj-1)-mdt(ji,nr+jj-1)
          enddo
        enddo
      ...
      if( .not. ln_sf ) THEN
         ...
         ENDIF
         CALL mpi_isend(south, imsn, mpi_double_precision, noso, tag2, mpi_comm_opa, isend_s,ierr)
         IF( llmpdebug ) THEN
          WRITE(numuni,*) ' Done'  ; CALL FLUSH(numuni)
         ENDIF
      endif
      ...
      if( .not. ln_nf ) THEN
         ...
         ENDIF
         CALL mpi_irecv(north,  imsn, mpi_double_precision, nono, mpi_any_tag, mpi_comm_opa, irecv_n, ierr)
         IF( llmpdebug ) THEN
          WRITE(numuni,*) ' Done'  ; CALL FLUSH(numuni)
         ENDIF
      endif
