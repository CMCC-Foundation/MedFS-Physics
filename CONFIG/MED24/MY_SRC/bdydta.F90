MODULE bdydta
   ...
   USE fldread_bdy     ! read input fields
   USE iom             ! IOM library
         ...
         IF( nn_dta(ib_bdy) .eq. 1 ) THEN ! skip this bit if no external data required
               ...
               IF( cn_dyn2d(ib_bdy) /= 'none' ) THEN
                  ...
                  IF (cn_tra(ib_bdy) /= 'runoff') THEN
                     IF( nn_dyn2d_dta(ib_bdy) .EQ. 1 .OR. nn_dyn2d_dta(ib_bdy) .EQ. 3 ) THEN
                        ...
                        CALL fld_read_bdy( kt=kt, kn_fsbc=1, sd=bf(jstart:jend), map=nbmap_ptr(jstart:jend),  &
            ...
            ELSE
               IF (cn_tra(ib_bdy) == 'runoff') then      ! runoff condition
                  ...
                  CALL fld_read_bdy( kt=kt, kn_fsbc=1, sd=bf(jstart:jend),  &
               ...
               IF(lwp .AND. kt .EQ. nit000 ) THEN
IF(lwp) WRITE(numout,*) '~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~'
IF(lwp) WRITE(numout,*) 'Read from BDY files with kt_offset=0'
IF(lwp) WRITE(numout,*) '~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~'
               ENDIF

                     CALL fld_read_bdy( kt=kt, kn_fsbc=1, sd=bf(jstart:jend), &
                                  & map=nbmap_ptr(jstart:jend), kt_offset=0 )
                  ENDIF
...
902         IF( ios /= 0 ) CALL ctl_nam ( ios , 'nambdy_dta in configuration namelist', lwp )
      ...
      DO ib_bdy = 1, nb_bdy
         ...
         CALL fld_fill_bdy( bf(jstart:jend), blf_i(jstart:jend), cn_dir_array(ib_bdy), 'bdy_dta',   &
