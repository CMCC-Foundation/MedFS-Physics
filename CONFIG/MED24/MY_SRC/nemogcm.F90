...
#if defined key_wwiii
   USE NemoWwCoupling  !Required to couple with WW [DRU]
   USE sbc_oce         !Required to couple with WW [DRU]
   USE par_kind        !Required to couple with WW [DRU]
#else
...
#if defined key_wwiii
      REAL(wp), DIMENSION(:,:), ALLOCATABLE :: CDS         ! mfs  Required to couple with WW [DRU]
      INTEGER                               :: i,j, ji,jj  ! mfs
#endif
      !!----------------------------------------------------------------------
      !
...
!      PRINT *, "DRU Start_test_36_nemo"
      !                            !-----------------------!
      CALL nemo_init               !==  Initialisations  ==!
      !                            !-----------------------!
#if defined key_wwiii
      PRINT *, "DRU Init nlci, nlcj, jpi, jpj: ",nlci,nlcj,jpi,jpj
      ALLOCATE(CDS(jpi,jpj)) !Required to couple with WW [DRU]
      CDS=0.0
      cdn_wave=0.0
#endif
...
#elif defined key_wwiii
IF(lwp) then
              WRITE(numout,*) 'in nemogcm cds_MINbef=',minval(CDS(1:nlci,1:nlcj))
              WRITE(numout,*) 'in nemogcm cds_MAXbef=',maxval(CDS(1:nlci,1:nlcj))
              WRITE(numout,*) 'in nemogcm cdn_waveMINbef=',minval(cdn_wave)
              WRITE(numout,*) 'in nemogcm cdn_waveMAXbef=',maxval(cdn_wave)
              WRITE(numout,*) 'in nemogcm unMINbef=',minval(un)
              WRITE(numout,*) 'in nemogcm unMAXbef=',maxval(un)
              WRITE(numout,*) 'in nemogcm dT_ASMINbef=',minval(dT_AS)
              WRITE(numout,*) 'in nemogcm dT_ASMAXbef=',maxval(dT_AS)
end if
!

             IF( MOD(istp,NINT(3600/rdttra(1))) == 1 ) THEN
               IF( (istp-nit000+1) == 1 ) THEN
                  IF(lwp) WRITE(numout,*) "RDTTRA is", rdttra(1)
                  IF(lwp) WRITE(numout,*) "[DRU] NemoWwHandshake to send u,v,t"
                  CALL NemoWwHandshake(sdt_n=dT_AS,sdu_n=un,sdv_n=vn,cds_n=CDS(1:nlci,1:nlcj),  &
                          gdxd=jpiglo,gdyd=jpjglo,  &
                          nsb=jpnij,sdxd=nlcit,sdyd=nlcjt,ldi=nimppt,ldj=njmppt)
                  IF(lwp) WRITE(numout,*) "[DRU] NemoWwHandshake to receive d"
                  CDS=0.0
                  CALL NemoWwHandshake(sdt_n=dT_AS,sdu_n=un,sdv_n=vn,cds_n=CDS(1:nlci,1:nlcj),  &
                          gdxd=jpiglo,gdyd=jpjglo,  &
                          nsb=jpnij,sdxd=nlcit,sdyd=nlcjt,ldi=nimppt,ldj=njmppt)
!
                  ELSE
                  CDS=0.0
                  IF(lwp) WRITE(numout,*) "[DRU] NemoWwHandshake to send u,v,t and receive d"
                  CALL NemoWwHandshake(sdt_n=dT_AS,sdu_n=un,sdv_n=vn,cds_n=CDS(1:nlci,1:nlcj),  &
                          gdxd=jpiglo,gdyd=jpjglo,  &
                          nsb=jpnij,sdxd=nlcit,sdyd=nlcjt,ldi=nimppt,ldj=njmppt)
!
...
!
             if(lwp)  then
           WRITE(numout,*) 'in nemogcm cds_MINbi2=',minval(CDS(1:nlci,1:nlcj))
           WRITE(numout,*) 'in nemogcm cds_MAXbi2=',maxval(CDS(1:nlci,1:nlcj))
                ENDIF

               DO jj = nlcj+1, jpj                 ! added line(s)   (inner only)
                   CDS(nldi  :nlei  , jj          ) = CDS(nldi:nlei,     nlej)
                   CDS(1     :nldi-1, jj          ) = CDS(nldi     ,     nlej)
                   CDS(nlei+1:nlci  , jj          ) = CDS(     nlei,     nlej)
                END DO
                DO ji = nlci+1, jpi                 ! added column(s) (full)
                   CDS(ji           ,nldj  :nlej  ) = CDS(     nlei,nldj:nlej)
                   CDS(ji           ,1     :nldj-1) = CDS(     nlei,nldj     )
                   CDS(ji           ,nlej+1:jpj   ) = CDS(     nlei,     nlej)
                END DO
!
          if (ln_cdgw) then
               cdn_wave(:,:)=CDS(:,:)
...
!
            ENDIF
!
            if (lwp) WRITE(numout,*) "ISTP,NIT000=",istp,nit000
!
             if(lwp)  then
           WRITE(numout,*) 'in nemogcm cds_MINaft=',minval(CDS(1:nlci,1:nlcj))
           WRITE(numout,*) 'in nemogcm cds_MAXaft=',maxval(CDS(1:nlci,1:nlcj))
              WRITE(numout,*) 'in nemogcm cdn_waveMINaft=',minval(cdn_wave)
              WRITE(numout,*) 'in nemogcm cdn_waveMAXaft=',maxval(cdn_wave)
              WRITE(numout,*) 'in nemogcm unMINaft=',minval(un)
              WRITE(numout,*) 'in nemogcm unMAXaft=',maxval(un)
              WRITE(numout,*) 'in nemogcm dT_ASMINaft=',minval(dT_AS)
              WRITE(numout,*) 'in nemogcm dT_ASMAXaft=',maxval(dT_AS)
             end if
!
            IF(lwp) WRITE(numout,*) "DRU before istp ",istp
            CALL stp( istp )                 ! standard time stepping
            IF(lwp) WRITE(numout,*) "DRU after istp ",istp
#elif defined key_mfs
            CALL stp( istp )
#else
            IF(lwp) WRITE(numout,*) 'If key_mfs is active you should not see this message'           
            CALL stp( istp )                 ! standard time stepping
#endif
...
!   SUBROUTINE MPI_ERRCHECK_TEST(ierr)
!
...
!
!      PRINT *, "DRU Start_test_37.1_nemo ",ierr," ",MPI_SUCCESS
!      IF (ierr /= MPI_SUCCESS) THEN
!         PRINT *, "DRU Start_test_37.2_nemo ",ierr," ",MPI_SUCCESS
!         CALL MPI_ERROR_STRING(ierr, str, str_len, ierr)
!         WRITE(*,FMT='(A)') TRIM(str)
!         CALL MPI_ABORT(MPI_COMM_WORLD, ierr, lerr)
!         END IF
!
!   END SUBROUTINE MPI_ERRCHECK_TEST
...
! Force values for AGRIF zoom (cf. agrif_user.F90)
   ...
   ENDIF
      ...
      IF( Agrif_Root() ) THEN
         ...
         ELSE
#if defined key_wwiii
!            PRINT *, "DRU Start_test_37_nemo"
            CALL mpi_init(code)
!            CALL MPI_ERRCHECK_TEST(code)
!            PRINT *, "DRU Start_test_38_nemo ",code,mpi_comm_world
            !CALL mpi_comm_dup( mpi_comm_world, ilocal_comm, code )
            !CALL MPI_ERRCHECK_TEST(code)
            ilocal_comm=mpi_comm_world
!            PRINT *, "DRU Start_test_39_nemo ",ilocal_comm
            CALL NemoWwInit(NemoWw_NEMO,3,ilocal_comm)
!            PRINT *, "DRU Start_test_40_nemo",ilocal_comm
            CALL  xios_initialize( "for_xios_mpi_id",local_comm=ilocal_comm ) 
!            PRINT *, "DRU Start_test_41_nemo"
#else
      ...
      ENDIF
      IF( ln_wrmppinit ) THEN
         WRITE(cfmpp,'(A,I4.4,A)') 'mppinit_',narea-1,'.out'
         OPEN(2099,FILE=cfmpp)
         WRITE(2099,'(12I5)')nproc,nowe,noea,nono,noso,nbondi,nbondj,npolj,nlci,nlcj,nimpp,njmpp
         CLOSE(2099)
      ENDIF
      !
